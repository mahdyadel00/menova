...
class SettingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->logo) {
            Storage::disk('local')->delete('public/uploads/images/' . setting('logo'));
            $request->logo->store('public/uploads/images');
            $requestData['logo'] = $request->logo->hashName();
        }

        if ($request->fav_icon) {
            Storage::disk('local')->delete('public/uploads/images/' . setting('fav_icon'));
            $request->fav_icon->store('public/uploads/images');
