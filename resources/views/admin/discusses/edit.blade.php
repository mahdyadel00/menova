    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="tile shadow">
                ...
                <form method="post" action="{{ route('admin.discusses.update', $discuss->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="{{$input}}" class="form-control" value="{{ old($input, $discuss->{$input} ) }}" required autofocus>
                    ...
                    <div class="form-group">
                        ...
                        @endphp
                        <label>@lang('discuss.'.$input)<span class="text-danger">*</span></label>
                        <textarea name="{{$input}}" class="form-control" id="editor" required autofocus>{{ old($input, $discuss->{$input} ) }}</textarea>
                        @error($input) <div class="text-danger">{{$message}}</div>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="{{$input}}" class="form-control select2" required>
                            ...
                            @foreach ($topics as $topic)
                                <option value="{{$topic->id}}" {{$topic->id == old($input, $discuss->{$input} ) ? 'selected' : ''}}>
                                    {{$topic->name}}
                                </option>
                            @endforeach
                        </select>
