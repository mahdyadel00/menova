            ...
            <div class="tile shadow">
                ...
                <form method="post" action="{{ route('admin.projects.update', $project->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="{{$input}}" class="form-control" value="{{ old($input, $project->{$input} ) }}" required autofocus>
                    ...
                    <div class="form-group">
                        ...
                        @endphp
                        <label>@lang('projects.'.$input)<span class="text-danger">*</span></label>
                        <textarea name="{{$input}}" class="form-control" id="editor" required autofocus>{{ old($input, $project->{$input} ) }}</textarea>
                        @error($input) <div class="text-danger">{{$message}}</div>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="{{$input}}" class="form-control select2" required>
                            ...
                            @foreach ($domains as $domain)
                                <option value="{{$domain->id}}" {{$domain->id == old($input, $project->{$input} ) ? 'selected' : ''}}>
                                    {{$domain->name}}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="{{$input}}" class="form-control select2" required>
                            ...
                            @foreach ($projectTypes as $pt)
                                <option value="{{$pt->id}}" {{$pt->id == old($input, $project->{$input} ) ? 'selected' : ''}}>{{$pt->name}}</option>
                            @endforeach
                        </select>
                        @error($input) <div class="text-danger">{{$message}}</div>@enderror
                    </div>
                    ...
                    <div class="form-group" id="users_list">
                        ...
                        <select name="{{$input}}" class="form-control select2" required>
                            <option value="{{$project->user_id}}">{{$project->user->full_name}}</option>
                        </select>
                        @error($input) <div class="text-danger">{{$message}}</div>@enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <img src="{{ $project->image_path }}" class="loaded-image" alt="" style="width: 100px; margin: 10px 0;">
                        @error($input) <div class="text-danger">{{$message}}</div>@enderror
                    </div>
