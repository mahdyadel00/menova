...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.0/font/bootstrap-icons.css">
    ...
    <div>
        <h2>@lang('counters.counters')</h2>
    </div>
    <ul class="breadcrumb mt-2">
        <li class="breadcrumb-item"><a href="{{ route('admin.home') }}">@lang('site.home')</a></li>
        <li class="breadcrumb-item"><a href="{{ route('admin.counters.index') }}">@lang('counters.counters')</a></li>
                            ...
                            <div class="tab-pane fade show {{$index == 0 ? 'active' : ''}} pt-2" id="{{$locale}}">
                                ...
                                <div class="form-group">
                                    @php
                                        $input = 'title';
                                    @endphp
                                    <label>@lang('counters.'.$locale.'.'.$input)<span class="text-danger">*</span></label>
                                    <input type="text" name="{{$locale.'_'.$input}}" class="form-control" value="{{ old($locale.'_'.$input, $counter->translate($locale)->{$input}) }}"  autofocus>
                                ...
                                <div class="form-group">
                                    ...
                                    @endphp
                                    <label>@lang('counters.'.$locale.'.'.$input)<span class="text-danger">*</span></label>
                                    <textarea name="{{$locale.'_'.$input}}" class="form-control" id="editor_{{$locale}}"  autofocus>{!! old($locale.'_'.$input, $counter->translate($locale)->{$input}) !!}</textarea>
                                    @error($locale.'_'.$input) <div class="text-danger">{{$message}}</div>@enderror
                                </div>
