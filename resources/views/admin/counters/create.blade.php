            ...
            <div class="tile shadow">
                ...
                <ul class="nav nav-tabs">
                    @foreach (config('translatable.locales') as $index => $locale)
                        <li class="nav-item"><a class="nav-link {{ $index == 0 ? 'active' : '' }}" data-toggle="tab"
                                href="#{{ $locale }}">{{ strtoupper($locale) }}</a></li>
                    @endforeach
                </ul>
                ...
                <form method="post" action="{{ route('admin.counters.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="tab-pane fade show {{ $index == 0 ? 'active' : '' }} pt-2" id="{{ $locale }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="{{ $locale . '_' . $input }}" class="form-control"
                                        value="{{ old($locale . '_' . $input) }}" autofocus>
                                    @error($locale . '_' . $input)
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="counter" class="form-control" autofocus>
                        @error('counter')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
